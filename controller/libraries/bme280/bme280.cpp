...
/***************************************************************************
 PRIVATE FUNCTIONS
 ***************************************************************************/
Adafruit_BME280::Adafruit_BME280(uint8_t bme_adress)
    : _cs(-1), _mosi(-1), _miso(-1), _sck(-1)
{ this->bme_adress = bme_adress; }
...
void Adafruit_BME280::write8(byte reg, byte value) {
    if (_cs == -1) {
        Wire.beginTransmission((uint8_t)bme_adress);
...
uint8_t Adafruit_BME280::read8(byte reg) {
    ...
    if (_cs == -1) {
        ...
        Wire.write((uint8_t)reg);
        Wire.endTransmission();
        Wire.requestFrom((uint8_t)bme_adress, (byte)1);
        value = Wire.read();
    } else {
        if (_sck == -1)
            SPI.beginTransaction(SPISettings(500000, MSBFIRST, SPI_MODE0));
...
uint16_t Adafruit_BME280::read16(byte reg)
{
    ...
    if (_cs == -1) {
        ...
        Wire.write((uint8_t)reg);
        Wire.endTransmission();
        Wire.requestFrom((uint8_t)bme_adress, (byte)2);
...
uint32_t Adafruit_BME280::read24(byte reg)
{
    ...
    if (_cs == -1) {
        ...
        Wire.write((uint8_t)reg);
        Wire.endTransmission();
        Wire.requestFrom((uint8_t)bme_adress, (byte)3);
